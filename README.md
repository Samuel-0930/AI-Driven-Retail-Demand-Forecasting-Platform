# DemandSense: AI 기반 수요 예측 포트폴리오

DemandSense는 **실데이터 EDA 사례**와 **재현 가능한 합성 데이터 예측 앱**을 분리해 보여 주는 데이터 분석 중심 포트폴리오입니다. 한쪽은 수요 구조를 비판적으로 해석하고, 다른 한쪽은 모델 학습부터 API·대시보드까지의 흐름을 직접 실행할 수 있게 합니다.
...
- FastAPI 입력 검증·오류 처리 및 Prometheus API 메트릭
...
이 명령은 합성 데이터가 없으면 시드 42로 생성하고, 기본 조합(매장 1·상품 1)의 모델을 학습합니다. 이후 [http://localhost:3000](http://localhost:3000)에서 동일 조합을 선택해 예측을 확인할 수 있습니다.
...
- 검증은 마지막 30일 holdout이고, 평가 시 실제 holdout 프로모션 값을 사용해 미래 성능보다 낙관적일 수 있습니다.
