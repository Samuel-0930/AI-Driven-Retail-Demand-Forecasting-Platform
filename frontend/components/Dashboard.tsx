...
import { api, ApiError, BacktestResponse, CommaxBacktestResponse, CommaxEvaluationResponse, CommaxItem, PredictionResponse } from '../lib/api';
...
export default function Dashboard() {
    ...
    const loadCommaxForecast = async () => {
        ...
        try {
            setCommaxBacktest(await api.getCommaxBacktest(commaxItemCode, 6));
        }
                        ...
                        {commaxEvaluation && <section className="rounded-xl border border-amber-200 bg-amber-50 p-6" aria-labelledby="commax-heading">
                            ...
                            <div className="mt-5 rounded-lg border border-amber-200 bg-white p-4">
                                <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
                                    ...
                                    <button type="button" onClick={() => void loadCommaxForecast()} disabled={commaxLoading || !commaxItems.length} className="rounded-lg bg-amber-700 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">{commaxLoading ? '불러오는 중…' : '실제값과 예측 비교'}</button>
                                </div>
                                {selectedCommaxItem && <p className="mt-2 text-xs text-slate-600">선택된 수요 패턴: <strong>{selectedCommaxItem.pattern}</strong></p>}
                                {commaxBacktest && <div className="mt-4"><p className="text-sm font-semibold text-slate-800">{commaxBacktest.item_code} · {commaxBacktest.pattern} · {modelName(commaxBacktest.champion)}<span className="font-normal text-slate-600"> · 최근 6개월 holdout WAPE {commaxBacktest.holdout_wape.toFixed(2)}%</span></p><div className="mt-3 h-[300px]"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={commaxBacktest.points}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="date" /><YAxis /><Tooltip formatter={(value, name) => [Number(value).toLocaleString(), name === 'actual' ? 'Actual shipments' : 'Forecast']} /><Legend /><Line dataKey="actual" name="actual" stroke="#0f766e" strokeWidth={3} dot /><Line dataKey="forecast" name="forecast" stroke="#b45309" strokeWidth={3} dot strokeDasharray="5 4" /></ComposedChart></ResponsiveContainer></div><p className="mt-2 text-xs text-slate-500">초록선은 실제 출하량, 주황 점선은 당시 학습 데이터만으로 계산한 예측값입니다.</p><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[620px] text-sm"><thead className="border-b border-slate-200 text-left text-slate-500"><tr><th className="pb-2 font-medium">월</th><th className="pb-2 text-right font-medium">실제 출하량</th><th className="pb-2 text-right font-medium">예측값</th><th className="pb-2 text-right font-medium">절대오차</th><th className="pb-2 text-right font-medium">오차율</th></tr></thead><tbody>{commaxBacktest.points.map((point) => <tr key={point.date} className="border-b border-slate-100 text-slate-700"><td className="py-2">{point.date}</td><td className="py-2 text-right">{point.actual.toLocaleString()}</td><td className="py-2 text-right">{point.forecast.toLocaleString()}</td><td className="py-2 text-right">{point.absolute_error.toLocaleString()}</td><td className="py-2 text-right">{point.actual ? `${(point.absolute_error / point.actual * 100).toFixed(1)}%` : '—'}</td></tr>)}</tbody></table></div></div>}
                            </div>
                        </section>}
                    </div>
                </div>
...
function modelName(name: string) {
    return { croston_sba: 'Croston/SBA', seasonal_croston_sba: 'Seasonal Croston/SBA', tsb: 'TSB', seasonal_naive: 'Seasonal-naive', prophet: 'Prophet' }[name] ?? name;
}
